...
相机命名依次使用机器人级映射、全局精确规则、正则规则和 VLM 语义候选。机器人身份已有相机拓扑约束、但源字段不在已验证映射中时，VLM 候选只写入复核文件，不会直接改名。相机位置无法确认不影响独立的模态与编码判断：可确认的 RGB 仍建议 AV1，Depth 仍建议 FFV1。
...
`info_norm_review.json` 的 `robot_identity` 保存最终型号、选用来源、全部候选证据和冲突。缺少可靠相机安装位证据时，源相机字段保留在 `info_norm.json`，并以 `ROBOT_CAMERA_MAPPING_UNKNOWN` 进入通用人工复核。
