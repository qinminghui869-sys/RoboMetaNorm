...
每个数据集执行两次本地 VLM 操作：第一次结合代表帧和机器 schema 生成 dataset-local
cameras/components profile，第二次将相机、`action` 和 `observation.state` 关联到已校验的
profile。两次调用均不进行联网硬件查询；最终名称、置信度、媒体属性、字段形状和机器
...
- `--vlm-max-tokens` 默认值为 `4096`，分别用于 profile 和 mapping 响应；
...
profile 或 mapping 未通过本地 schema 校验时，复核记录分别使用
`DATASET_PROFILE_INVALID` 或 `DATASET_MAPPING_INVALID`，并在 `evidence` 中给出
`stage`、`error_type` 和不含原始响应值的 `validation_error`，用于定位失败边界。
