...
复核状态下，YAML 可保留已规范的相机键，以及根据 `action` 与
`observation.state` 的一致、连续结构得到的本地尽力推导（best-effort）机器通道。
这些通道仅用于支持后续复核，不代表已经 VLM 确认。

相机键采用 `observation.images.cam_<位置>_<rgb|depth>`，其值保留原始相机字段。机器
